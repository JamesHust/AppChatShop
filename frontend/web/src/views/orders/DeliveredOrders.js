...
const DeliveredOrders = () => {
  ...
  const [listDeliveredOrder, setListDeliveredOrder] = useState([]);
  ...
  const getDeliveredOrders = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://192.168.1.125:3000/api/admin/orders?status=3&shopId=${admin.shopId}`,
        {
          method: "GET",
          headers: {
      ...
      switch (response.status) {
        case 200:
          ...
          setListDeliveredOrder(resData.data);
          setIsLoading(false);
          return;
        case 404:
          setListDeliveredOrder([]);
          setIsLoading(false);
          return;
        default:
          setIsLoading(false);
  ...
  const DetailModal = () => {
    ...
    const deleteDeliveryOrder = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(
          `http://192.168.1.125:3000/api/delivery/${dataModal.orderId}`,
          {
            method: "DELETE",
            headers: {
              Accept: "application/json",
    ...
    const handlerConfirmOrder = async () => {
      ...
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(
          `http://192.168.1.125:3000/api/orders?status=4&orderId=${dataModal.orderId}`,
          {
            method: "PUT",
            headers: {
              Accept: "application/json",
