...
const FailedOrders = () => {
  ...
  const [listFailedOrders, setListFailedOrders] = useState([]);
  ...
  // Hàm thực hiện lấy danh sách hóa đơn thất bại
  const getFailedOrders = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://192.168.1.125:3000/api/admin/orders?status=5&shopId=${admin.shopId}`,
        {
          method: "GET",
          headers: {
      ...
      switch (response.status) {
        case 200:
          ...
          setListFailedOrders(resData.data);
          setIsLoading(false);
          return;
        case 404:
          setListFailedOrders([]);
        ...
        default:
          setIsLoading(false);
          alert("Lỗi lấy danh sách hóa đơn đã thất bại.");
          return;
      }
    } catch (err) {
      setIsLoading(false);
          ...
          <div>
            ...
            <div className="mt-3">
              ...
              <div className="mt-2">
                ...
                <CTextarea
                  defaultValue={dataModal.reason ? dataModal.reason.Reason : "Không có lý do"}
                  disabled
                  rows={4}
                  className="mt-1"
                />
