...
const WaitingOrders = () => {
  ...
  const [listWaitOrders, setListWaitOrders] = useState([]);
  ...
  // Hàm thực hiện lấy danh sách hóa đơn đợi
  const getWaitOrders = useCallback(async () => {
    setIsLoading(true);
    //fetching data ở đây
    try {
      ...
      switch (response.status) {
        case 200:
          ...
          setListWaitOrders(resData.data);
          setIsLoading(false);
          return;
        case 404:
          setListWaitOrders([]);
        ...
        default:
          setIsLoading(false);
          alert("Lỗi lấy danh sách hóa đơn đợi");
          return;
      }
    } catch (err) {
      setIsLoading(false);
      alert(`Lỗi tải danh sách hóa đơn đợi: ${err}`);
    }
  }, [admin]);
    ...
    return (
      ...
      >
        ...
        <CModalBody>
          ...
          <div>
            <div style={{ backgroundColor: COLORS.grey_3 }} className="mt-3">
              <div style={{ fontWeight: "bold" }} className="px-2 py-2">
        ...
        <CModalFooter>
          <CButton color="info" shape="pill" onClick={handlerReceiveOrder}>
            Nhận hóa đơn
          </CButton>
          <CButton
            color="secondary"
